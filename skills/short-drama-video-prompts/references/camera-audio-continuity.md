...
**`craft_default`**：单镜产物默认只承载同期对白、画外声源、环境底声与事件音效；配乐
不写进单镜声音，也不让单镜产物自带配乐床。

理由是连续性方向：配乐的进入、退出与层级跨越多个镜头，是时间线上的决定；一旦每个镜头
各自带一段音乐，剪辑点就会出现无法对齐的硬切，而这个缺陷在单镜文本里看不出来——它只在
镜头相接处暴露。把音乐留在时间线层，单镜之间才可以自由调换、补拍或替换。

- 若项目已接受“单镜自带音乐”的做法（例如整集只有一个容器，或音乐本身是画内声源，如
  收音机、现场演奏、手机铃声），按已接受方案写，并注明它是画内声源还是配乐层；画内
  声源属于 SFX/ambience 的范畴，不受本条约束。
