...
**`reviewed_invariant`**：本镜必须被读到的内容（脸与眼神、证据上的可读文字、决定性的
手部动作）不能只落在交付面会被叠加遮挡的区域里；哪些区域被占用由创作者已接受的交付
方案声明，本文不假定具体数值。

- **先向创作者取回答**，不要猜：这次交付的播放面会在画幅的哪些位置叠加什么、是否常驻。
  没有声明时按"上下两端与一侧边缘可能被占用"保守处理，并把它记为未解决，而不是当成
  没有约束。
