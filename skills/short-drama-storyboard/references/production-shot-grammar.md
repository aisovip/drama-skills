...
| 单镜还是镜头组 | 先把每个镜头写成可单独审查的记录，再按目标交付方式编组；编组不改变镜头身份。编组后的时长算术与成员资格由视频提示词环节按[交付容器](../../short-drama-video-prompts/references/delivery-profile.md#二容器算术唯一允许的数字规则)处理，本环节只负责镜头本身。 |
