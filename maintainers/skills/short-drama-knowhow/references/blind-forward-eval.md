...
4. 保存 prompt、skill 版本 hash、输入 hash、输出、工具调用摘要和未完成原因。
...
揭盲后，把以下材料交给独立 reviewer：匿名输出及盲评、版本映射、公共候选包、无敏感内容的证据摘要、
泄漏扫描结果、冲突与边界清单。只有在需要复核隐私证明且 reviewer 同样获得授权时，才单独提供隔离映射；
不得把私有卡复制进公共评测记录。
